...
public class Queries {
	...
	public static Q callEdgesContext = Common.resolve(null, Common.universe().edgesTaggedWithAny(XCSG.Call));
	...
	public static  Q global(String name){
		return Common.universe().nodesTaggedWithAll(XCSG.GlobalVariable).selectNode(XCSG.name, name);
	}
	...
	public static  Q Type(String name){
		return Common.universe().nodesTaggedWithAll(XCSG.C.Struct).selectNode(XCSG.name, "struct " + name);
	}
	...
	public static  Q refVariable(Q variable){
		Q read = Common.universe().edgesTaggedWithAny(XCSG.ReadsVariable, XCSG.Reads, XCSG.ReadsFunctionAddress, XCSG.readOnly).reverseStep(variable);
...
//	}
	...
	Q efg = Common.universe().edgesTaggedWithAny(XCSG.Contains, EVENT_FLOW_EDGE).forward(method).nodesTaggedWithAny(EVENT_FLOW_NODE).induce(Common.universe().edgesTaggedWithAll(EVENT_FLOW_EDGE));
	return efg;
}
	...
	public static  Q argto(Q function, int index){
		return CommonQueries.functionParameter(function, 0).roots();
	}
	...
	public static  Q functionReturn(Q functions) {
		return Common.edges(XCSG.Contains).forwardStep(functions).nodesTaggedWithAll(XCSG.MasterReturn);
	}
	...
	public static  Q getFunctionContainingElement(Q q){
		Q declares = Common.edges(XCSG.Contains).reverseStep(q);
		declares = Common.edges(XCSG.Contains).reverseStep(declares);
		return declares.nodesTaggedWithAll(XCSG.Function, "isDef");
	}
	
	public static  Q getFunctionContainingElement(Node e){
		return getFunctionContainingElement(Common.toQ(e));
	}
	...
	public static boolean isCalling(Node node, HashSet<String> functions){
	...
	public static  boolean isCalling(Node node, HashSet<String> functions, FileWriter writer){
		for(String f : functions){
			try {
	...
	public static  void deleteEFGs(){
		AtlasSet<Edge> edges = Common.universe().edgesTaggedWithAll(EVENT_FLOW_EDGE).eval().edges();
		...
		AtlasSet<Node> nodes = Common.universe().nodesTaggedWithAll(EVENT_FLOW_NODE).eval().nodes();
	...
	public static Object[] isQualified(Q method){
		...
		Q callsites = Common.universe().edgesTaggedWithAny(XCSG.InvokedFunction, XCSG.InvokedSignature).predecessors(lockQ).nodesTaggedWithAll(XCSG.CallSite);
		Q LcfgNodes = Common.universe().edgesTaggedWithAll(XCSG.Contains).reverseStep(callsites).nodesTaggedWithAll(XCSG.ControlFlow_Node);
		LcfgNodes = LcfgNodes.intersection(cfg);
		
		callsites = Common.universe().edgesTaggedWithAny(XCSG.InvokedFunction, XCSG.InvokedSignature).predecessors(unlockQ).nodesTaggedWithAll(XCSG.CallSite);
		Q UcfgNodes = Common.universe().edgesTaggedWithAll(XCSG.Contains).reverseStep(callsites).nodesTaggedWithAll(XCSG.ControlFlow_Node);
