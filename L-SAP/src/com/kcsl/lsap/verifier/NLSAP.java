...
import com.kcsl.lsap.atlas.scripts.Queries;
...
public class NLSAP {
	...
	public static String locate(String lock){
		Q callsites = universe().nodesTaggedWithAll(XCSG.CallSite);
		Q cfgNodes = universe().edgesTaggedWithAll(XCSG.Contains).predecessors(callsites);
	...
	public static Q verify(Q selectedLockInstance){
		...
		Q parametersPassedtoLockInstance = universe().edgesTaggedWithAll(XCSG.PassedTo).reverseStep(selectedLockInstance);
		...
		Q dataFlowEdges = universe().edgesTaggedWithAny(XCSG.DataFlow_Edge, Edge.ADDRESS_OF, Edge.POINTER_DEREFERENCE);
	...
	private static Q getReachableCallsites(Q lockInstance, Q lockField){
		...
		Q parameters = methodParameter(lockUnlockMethods, 0);
		Q dataFlowEdges = universe().edgesTaggedWithAny(XCSG.DataFlow_Edge, Edge.ADDRESS_OF, Edge.POINTER_DEREFERENCE);
		...
		Q reachableCallsites = universe().edgesTaggedWithAll(XCSG.PassedTo).forwardStep(forwardDataFlow.leaves());
